...
class Cli {
	...
	public function demo_install ($args, $assoc_args) {
		...
		$progress = \WP_CLI\Utils\make_progress_bar( 'Overall Progress', count($commands) + 1 );
	...
	public function demo_list ($args, $assoc_args) {
		...
		foreach ($demo_data as $demo) {
			...
			$merged_data[$id]['categories'] = array_unique(array_merge($merged_data[$id]['categories'], !empty($demo['categories']) ? $demo['categories'] : []));
			$merged_data[$id]['keywords'] = array_unique(array_merge($merged_data[$id]['keywords'], explode(', ', !empty($demo['keywords']) ? $demo['keywords'] : '')));
			...
			$merged_data[$id]['plugins'] = array_unique(array_merge($merged_data[$id]['plugins'], !empty($demo['plugins']) ? $demo['plugins'] : []));
		}
		...
		$fields = array_filter(explode(',', \WP_CLI\Utils\get_flag_value( $assoc_args, 'fields', implode(',', $known_fields))), function($field) use ($known_fields) {
	 ...
	 * @return array
	 */
	private function get_demo_args($args) {
		if (empty($args)) {
			\WP_CLI::error('Please provide demo name.');
		}

		if (!isset($args[1]) || $args[1] == 'gutenberg') {
			$args[1] = '';
		}
